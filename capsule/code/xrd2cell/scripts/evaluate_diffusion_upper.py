...
class TimeoutExpection(Exception):
    pass
    ...
    for idx, batch in enumerate(iter(loader)):
        ...
        for eval_idx in range(num_evals):
            ...
            for j in tqdm(range(len(num_L_lattice_tensor))):
                error=1000000000
                induce=0
                cell_true = true_batch_lattice[j]    
                for k in range(len(num_L_lattice_tensor[j])):
                    try:
                        # signal.signal(signal.SIGALRM, timeout_handler)
                        # signal.alarm(180)
                        cell_pre = num_L_lattice_tensor[j][k]
                        test_error = torch.abs(cell_pre - cell_true).mean()
                        if error >= test_error:
                            error = test_error
                            induce = k
                    except:
...
def main(args):
    ...
    if args.label == -1:
        save_name = 'last_sample%s_L%s_upper.pt'%(str(num_evals), str(num_L_sample))     ######last_one.ckpt
    else:
        save_name = 'best_sample%s_L%s_upper.pt'%(str(num_evals), str(num_L_sample))     ######best_one.ckpt
