...
def load_model(model_path, label=-1):
    with initialize_config_dir(str(Path(model_path).resolve()/'.hydra')):
...
def flow(loader, model, step_lr = 5, infer_timesteps=200):
...
def main(args):
    ...
    (frac_coords, atom_types, lattices, lengths, angles, num_atoms) = flow(test_loader, diff_model)
    ...
    if os.path.exists('/workspace/PXRDgen/capsule-7727770/code/xrd2struc/samples/%s_target.cif'%args.formula):
        print('The target file exists, try to compare the predicted files with the target file:')
        s1 = Structure.from_file('/workspace/PXRDgen/capsule-7727770/code/xrd2struc/samples/%s_target.cif'%args.formula)
        predict_files = '/workspace/results/%s'%args.formula
