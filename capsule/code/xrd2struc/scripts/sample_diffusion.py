...
def diffusion(loader, model, step_lr=1e-5):
    ...
    for idx, batch in enumerate(loader):
        ...
        if torch.cuda.is_available():
            batch.cuda()
        outputs = model.sample(batch, step_lr = step_lr)
...
def main(args):
    ...
    (frac_coords, atom_types, lattices, lengths, angles, num_atoms) = diffusion(test_loader, diff_model)
