...
from pxrdgen.model.diff_utils import d_log_p_wrapped_normal, lattice_params_to_matrix_torch
import hydra
...
class CSPDiffusion(BaseModule):
    ...
    def forward(self, batch):
        ...
        times = self.beta_scheduler.uniform_sample_t(batch_size, self.device)    # (batch_size)
        ...
        sigmas = self.sigma_scheduler.sigmas[times]                             # (batch_size)
        sigmas_norm = self.sigma_scheduler.sigmas_norm[times]                   # (batch_size)
        ...
        encoded_xrd = self.xrd_encoder(batch.xrd_array)    ## batch.xrd_array (batch_size, 7500) --> encoded_xrd (batch_size, latent_size)
    ...
    def sample(self, batch, step_lr = 1e-5):
        ...
        for t in tqdm(range(time_start, 0, -1)):
            ...
            pred_l, pred_x = self.decoder(time_emb, batch.atom_types, x_t_minus_05, l_t_minus_05, batch.num_atoms, batch.batch, encoded_xrd)
    ...
    @torch.no_grad()
    def sample_given_inital_cell(self, batch, initial_cell, step_lr = 1e-5):
